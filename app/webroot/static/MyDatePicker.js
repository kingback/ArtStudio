...
function get_value() {
    return get_element(arguments[0]).value;
}
...
function fPopCalendar(evt, popCtrl, dateCtrl) {
    ...
    with(get_element("calendardiv").style) {
        left = point.x + "px";
...
function fHideCalendar() {
    get_element("calendardiv").style.visibility = "hidden";
...
function fSetSelected() {
    var iOffset = 0;
    var iYear = parseInt(get_element("tbSelYear").value);
    var iMonth = parseInt(get_element("tbSelMonth").value);
    var aCell = get_element("cellText" + arguments[0]);
...
function fSetYearMon(iYear, iMon) {
    get_element("tbSelMonth").options[iMon - 1].selected = true;
    for (var i = 0; i < get_element("tbSelYear").length; i++) {
        if (get_element("tbSelYear").options[i].value == iYear) {
            get_element("tbSelYear").options[i].selected = true;
        }
    }
...
function fPrevMonth() {
    var iMon = get_element("tbSelMonth").value;
    var iYear = get_element("tbSelYear").value;
    if (--iMon < 1) {
        iMon = 12;
        iYear--;
    }
...
function getDateDiv() {
    ...
    dateDiv += "</td><td><select id='tbSelYear' style='border:1px solid;' onchange='fUpdateCal(get_value(\"tbSelYear\"),get_value(\"tbSelMonth\"))'>";
    for (var i = startYear; i < endYear; i++) {
        dateDiv += "<option value='" + i + "'>" + i + strYear + "</option>";
    }
    dateDiv += "</select></td><td>";
    dateDiv += "<select id='tbSelMonth' style='border:1px solid;' onchange='fUpdateCal(get_value(\"tbSelYear\"),get_value(\"tbSelMonth\"))'>";
    for (var i = 0; i < 12; i++) {
